...
if (browser) {
	...
	const json = JSON.parse(localStorage.getItem('ll-accounts') || '[]');
	if (json.length) {
		manager.fromJSON(json);

		// load active account from storage
		const active = localStorage.getItem('ll-active');
		if (active) manager.setActive(active);

		// subscribe to active changes
	}
	manager.active$.subscribe((account) => {
		if (account) localStorage.setItem('ll-active', account.id);
		else localStorage.removeItem('ll-active');
	});
	// next, subscribe to any accounts added or removed
	manager.accounts$.subscribe(() => {
		// save all the accounts into the "accounts" field
		localStorage.setItem('ll-accounts', JSON.stringify(manager.toJSON()));
	});
}
